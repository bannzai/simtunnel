...
9. **runner スクリプトは workflow と同一 commit に固定**: reusable workflow（session.yml）は runner スクリプトを `github.job_workflow_sha`（= 呼ばれた workflow ファイルの commit SHA）で checkout する。caller が `uses:` を SHA 固定していれば、実行されるスクリプトも同じ SHA に固定される
...
- OIDC token の subject は **caller repo 基準**になるため、アプリ repo ごとに Tailscale trust credential の発行（Subject を `repo:<owner>/<repo>:*` にする）と Secrets（`TS_OIDC_CLIENT_ID` / `TS_OIDC_AUDIENCE`）の登録が必要（「Tailscale セットアップ手順」の subject 読み替え）
...
- [ ] Tailscale trust credential の subject ワイルドカード（`repo:bannzai/*` 等）検証。効けば 1 credential + 同一 Secrets で全 repo をカバーできる。効かなければ repo ごとに発行
- [ ] SwiftUI 実験 repo（SimTunnelDemoProject）で caller workflow + credential + Secrets をセットアップし、up → mcp-config → 操作の一連を通す
