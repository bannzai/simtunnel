...
設計は「public リポジトリでも安全」を基準にする。現在の運用は private（macOS Runner の課金 10 倍は了承済み）だが、公開しても破綻しない水準を維持し、tailnet 内の実 IP 等の環境固有情報はこのリポジトリに書かない。
...
- [x] リポジトリを GitHub に作成（現在は private 運用。macOS Runner の課金 10 倍は了承済み）
